...
import { drawResults, getDetector } from "@/utils/midia-pipe-detect";
...
const SystemCheck = () => {
  ...
  const handleTryAgain = () => {
    isLightSuccess.current = false;
    count.current = 5;
    setSystemData(initialSysData);
    setReset((val) => !val);
  ...
  const loadDetector = async () => {
    try {
      const myDetector = await getDetector();
      ...
      if (videoEl.current) {
        ...
        if (videoEl.current.currentTime !== lastVideoTime.current) {
          ...
          if (results?.[0]) {
            ...
            if (systemData.light.status === "loading") {
              setSystemData((data) => ({
                ...data,
                ["light"]: {
                  ...data["light"],
          ...
          console.log({ count: count.current, startCount: startCount.current });
          if (count.current === 0) {
            if (!isLightSuccess.current) {
              setSystemData((data) => ({
                ...data,
                ["light"]: {
                  ...data["light"],
          ...
          //        drawResults(context, [result], true, false);
        } else {
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timer && timer);
  }, []);
  ...
  useEffect(() => {
    ...
    if (start) {
      timeout = setTimeout(() => {
        ...
        testInternetSpeed()
          .then((result) => {
            console.log("internet success", result);
            setSystemData((data) => ({
              ...data,
              ["internet"]: {
                ...data["internet"],
        ...
        initiateWebCam()
          .then((stream) => {
            console.log("webcam success", stream);
            setSystemData((data) => ({
              ...data,
              ["webCam"]: {
                ...data["webCam"],
  ...
  return (
    <>
      <div className="flex">
        <div className=" w-2/5">
          <div className="w-[275px] border border-[#755AE2] bg-[#F5F5F5] rounded-[10px] overflow-hidden">
      ...
      {checkFinished() && checkAllSuccess() ? (
        <button
          onClick={() => {
            setOpen(true)
          }}
          className="py-[13px] px-[17px] bg-[#755AE2] text-white mt-[40px] rounded-lg"
        >
          System Checks Successful
        </button>
      ) : checkFinished() ? (
        <button
          onClick={handleTryAgain}
          className="py-[13px] px-[17px] bg-[#FF5F56] text-white mt-[40px] rounded-lg"
        >
